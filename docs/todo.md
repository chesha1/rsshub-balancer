...
当前 [公开代理入口](../apps/server/src/app.ts) 的前置直转分支在 `recordRouteRequestMetric()` 之前返回，成功和失败请求均未进入桑基图。完整选路内部的 fallback 和重试仍会走现有指标记录位置，不属于这项漏记。
...
- [定时健康检查](../apps/server/src/scheduled.ts)和[应用接口](../apps/server/src/app.ts)：只读取状态码的健康检查，以及 `/api/route/status` 中失败或未被 `Promise.any` 选中的已返回响应。
