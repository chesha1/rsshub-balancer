...
服务端及必要首页适配已完成本地实现，线上启用仍待执行，见[实施状态](./implementation-status.md)。两端使用新数据集 `rsshub_balancer_request_flows`：Node 后台批量上传到 Worker ingest，由 binding 写入；Worker 接管时直接调用 `METRICS.writeDataPoint()`。两端共享 HTTP SQL 查询，接口归属见[路由](./README.md#路由)。
