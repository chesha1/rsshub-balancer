...
| 指标 | WAF 允许服务器公网出口 IP、阻断其他来源访问 ingest，且无绕过 WAF 的公开入口；固定字段映射正常，两端字段与统计口径一致；上传失败不影响 RSS；查询同一数据集，新旧数据和前端兼容 |
