...
新版本失败时保持 Worker 接管，恢复旧 Node 镜像和配置后重验；不清空两端 Redis 状态或历史指标。容器信号必须到达 Node，`stop_grace_period` 稍长于应用总退出时限。

业务改动默认用同一源码配套构建、验证两端，适配层修复可在兼容时单侧发布。允许新旧并存：指标升级先部署兼容新旧格式的 Worker 接收端，再更新 Node；回滚接收端前须先恢复兼容发送端，并核对凭据。查询结构和前端配套发布、回滚。
...
| 运行时 | 配置与双目标构建、缓存刷新/失败标记、Redis 隔离/复用/断线/超时及退出边界通过；核对可信 header、真实 IP 和反代来源限制 |
| 指标 | 非法批次零写入；两端字段与统计口径一致；上传失败不影响 RSS；查询同一数据集，新旧数据和前端兼容 |
