...
云下 Node 已部署，主域名于 2026-09-22 删除 Worker catch-all，完成首次切流。2026-09-23 用户确认线上运行良好，已通过各类 Dashboard 完成验收，详见[实施状态](./implementation-status.md)。目标是减少 Workers CPU Time 和费用，让已有服务器上的单进程 Node/Hono 承接日常 RSS，使用 Docker Compose + Traefik，保留 Cloudflare 橙云及现有 Worker 作为备用。首次迁移已手动完成；独立探活 Worker 已实现自动接管和恢复，待部署启用。维护用的手动接管／恢复 Actions 尚待实现。

## 接口归属

2026-09-22 用户已人工核验 Full (strict)、源站可信来源限制及 ingest WAF 白名单，并确认完成 DNS、重定向和缓存配置；API 读回确认仅保留三条长期 Routes，catch-all 已删除。2026-09-23 公开桑基图查询已有最近 24 小时的数据，线上验收按用户 Dashboard 核验结果完成。不再追加迁移专项验收或独立恢复演练，下一步部署启用探活 Worker 并实现接管／恢复 Actions，见[后续工作](./implementation-status.md#后续工作)。
...
业务查询与 RSS 一起切换，前端始终请求同域相对路径。Node 故障但尚未接管时，首页可打开，动态查询显示暂不可用。`/healthz` 保留上游聚合语义：至少一个上游在单次 5 秒探测内健康返回 200，否则 503；不用于容器重启。两端应用额外设置 `X-RSSHub-App: origin/edge`，供 watchdog 确认响应来自哪个应用。watchdog 每 3 分钟检查健康接口，再并行检查 `FEED_PATHS` 中的 `/openai/news` 与 `/github/issue/DIYgod/RSSHub` RSS 正文，全部通过才算健康，共用 30 秒上限；未接管时检查主域名，接管时检查固定源站，每轮最多切换一次，切换后的业务状态留给后续轮次判断。业务探针只代表所选路由，不能代替新版本的完整功能验收，具体见[自动接管与恢复](./migration-failover-runbook.md#后续自动化)。
...
固定云下验证入口为 `rsshub-balancer-origin.virworks.moe`，橙云指向 Oracle，由 Traefik 转发到同一个 Node 容器，不绑定 Worker。该入口用于主域名由 Worker 承接时检查 Node，只提供后端；其验收不能代替主域名规则检查。当前公开可用性见实施状态。
...
Worker 由 `apps/edge/src/index.ts` 进入共享应用，保留前后端组合部署：构建依赖 Web，Cloudflare Vite 插件将首页产物复制到 `dist/apps/edge/public`，脚本与按需加载的 Redis SDK 模块位于并列的 `server/`；发布使用该目录生成的 Wrangler 配置。`not_found_handling=none`，缺失静态文件返回 404。`run_worker_first` 保留内部接口、API、健康检查及 `robots.txt` 的脚本优先设置，Hono catch-all 保留备用 RSS 后端；Node 通过 `apps/origin/src/index.ts` 使用同一应用，只部署后端。这些资产设置与域名 Routes 分别管理。
