import { recordMetric } from '../adapters/metrics-worker'
import { metricsLogger } from '../log'
import type { MetricsBinding, RouteRequestMetric } from '../metrics-schema'

export const METRICS_INGEST_PATH = '/_internal/metrics/ingest'

type MetricsIngestOptions = {
...
export async function handleMetricsIngest(
  request: Request,
  options: MetricsIngestOptions,
): Promise<Response> {
  ...
  try {
    for (const event of batch.events) {
      recordMetric(options.metrics, event, 'origin')
    }
  } catch {
